...
def FuncSCT():
	...
	dateStr = "{:0>4d}-{:0>2d}-{:0>2d}".format(int(rtc.datetime()[0]), int(rtc.datetime()[1]), int(rtc.datetime()[2]))
	timeStr = "{:0>2d}:{:0>2d}:{:0>2d}".format(int(rtc.datetime()[4]), int(rtc.datetime()[5]), int(rtc.datetime()[6]))
	# Put string to OLED
	oled.text(dateStr, 0, 0)	# (message, x, y, color)
	oled.text(weekday[int(rtc.datetime()[3])], 0, 11)
...
def FuncChgT(changeTime):
	# Covert <int> list to <str>
	dateStr = "{:0>4d}-{:0>2d}-{:0>2d}".format(changeTime[0],changeTime[1],changeTime[2])
	timeStr = "{:0>2d}:{:0>2d}:{:0>2d}".format(changeTime[3], changeTime[4], changeTime[5])
	# Put string to OLED
	oled.text(dateStr, 0, 0)	# (message, x, y, color)
			...
			if Debounce(switchC):
				...
				if item == 0:	
					...
					while 1:
						...
						if switchA.value() == 0:
							if Debounce(switchA): 
								rtc.datetime((changeTime[0], changeTime[1], changeTime[2], 1, changeTime[3], changeTime[4], changeTime[5], 0))
								break
							...
							if Debounce(switchA): 
								...
								if changeTime[1]-int(rtc.datetime()[1]) > 0: # Not carry to another year
									for i in range(changeTime[1]-int(rtc.datetime()[1])):
										monthChangeDays += monthdays[int(rtc.datetime()[1])+i]
								else: # carry to another
									for i in range(int(rtc.datetime()[1])-changeTime[1]):
										monthChangeDays = monthChangeDays - monthdays[changeTime[1]+i]
								# then translate it to changed secs
								alarmTime = time.time() + ((changeTime[0]-int(rtc.datetime()[0]))*31536000+monthChangeDays*86400+(changeTime[2]-int(rtc.datetime()[2]))*86400+(changeTime[3]-int(rtc.datetime()[4]))*3600+(changeTime[4]-int(rtc.datetime()[5]))*60+(changeTime[5]-int(rtc.datetime()[6])))
								break
