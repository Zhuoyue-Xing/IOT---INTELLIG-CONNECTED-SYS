# Created by Chenye Yang, Haokai Zhao, Zhuoyue Xing on 2019/10/20.
...
def SendAWS(label, ID, seq, x1, x2, y1, y2, z1, z2):
	...
	url = "http://3.87.68.197:8099/post"
	request_data = {
	"label":label,
	"ID":ID,
	"seq":seq, 
...
def Interrupt(pin):
	if Debounce(switchA):
		...
		for seq in range(20):
			...
			SendAWS("C",ID,seq,accRaw[0],accRaw[1],accRaw[2],accRaw[3],accRaw[4],accRaw[5])
